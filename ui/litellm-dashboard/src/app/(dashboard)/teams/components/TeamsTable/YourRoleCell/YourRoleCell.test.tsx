...
vi.mock(
  "@/app/(dashboard)/teams/components/TeamsTable/YourRoleCell/TeamRoleBadge",
  () => ({
    __esModule: true,
    default: (role: string | null) => (
      <span data-testid="badge">{role === "admin" ? "Admin" : "Member"}</span>
    ),
  }),
);
...
): Team => ({ members_with_roles: members }) as unknown as Team;

describe("YourRoleCell", () => {
  it("renders Admin when the user is an admin of the team", () => {
    render(
      <YourRoleCell
        team={team([{ user_id: "u1", role: "admin" }])}
        userId="u1"
      />,
    );
  ...
  it("renders Member when the user is a regular member", () => {
    render(
      <YourRoleCell
        team={team([{ user_id: "u2", role: "user" }])}
        userId="u2"
      />,
    );
  ...
  it.each<[string, Team, string | null]>([
    ...
    "falls back to Member when no role can be determined (%s)",
    (_label, t, uid) => {
