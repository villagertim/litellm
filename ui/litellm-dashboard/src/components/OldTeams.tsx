...
import {
  type TeamsResponse,
  teamListCall as v2TeamListCall,
...
import { type Member, teamCreateCall } from "./networking";
    ...
    try {
      const response: TeamsResponse = await v2TeamListCall(
        ...
        size,
        {
          ...
          search: search || null,
          userID:
            userRole !== "Admin" && userRole !== "Admin Viewer" ? userID : null,
          sortBy: sortBy || null,
    ...
    () => [
      ...
      },
      {
        ...
        render: (_: unknown, record: Team) => {
          const orgAlias = getOrganizationAlias(
      ...
      },
      {
        ...
        render: (_: unknown, record: Team) => {
          const memberCount =
            perTeamInfo?.[record.team_id]?.team_info?.members_with_roles
              ?.length ?? 0;
        ...
        render: (_: unknown, record: Team) => {
          ...
          const percent =
            budgetVal != null && budgetVal > 0
              ? Math.min((spendVal / budgetVal) * 100, 100)
              : null;
          return (
            <Flex vertical gap={2}>
            ...
            >
              <Flex gap={12} align="center">
                <Input
                  ...
                  suffix={
                    isSearching ? <AntDLoadingSpinner size="small" /> : null
                  }
        ...
        >
          ...
          >
            <>
              ...
              {(() => {
                const adminOrgs = getAdminOrganizations(
                  userRole,
                  userID,
                  organizations,
                );
                      ...
                      label={
                        <span>
                          ...
                          <Tooltip
                            title={
                              <span>
                                Organizations can have multiple teams. Learn
                                more about{" "}
                                <a
                    ...
                    {isOrgAdmin && !isSingleOrg && adminOrgs.length > 1 && (
                      <div className="mb-8 p-4 bg-blue-50 border border-blue-200 rounded-md">
                        <Text style={{ color: "#1e40af", fontSize: 14 }}>
                          Please select an organization to create a team for.
                          You can only create teams within organizations where
                          you are an admin.
                        </Text>
                      </div>
                    )}
