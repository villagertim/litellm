...
export function useMarginConfig({
  accessToken,
}: UseMarginConfigProps): UseMarginConfigReturn {
      ...
      try {
        ...
        if (response.ok) {
          NotificationsManager.success(
            "Margin configuration updated successfully",
          );
        ...
        NotificationsManager.fromBackend(
          "Failed to update margin configuration",
        );
      }
    },
  ...
  const handleAddMargin = useCallback(
    async (params: AddMarginParams): Promise<boolean> => {
      ...
      if (marginType === "percentage") {
        const percentValue = Number.parseFloat(percentageValue);
        ...
        const fixedValue = Number.parseFloat(fixedAmountValue);
