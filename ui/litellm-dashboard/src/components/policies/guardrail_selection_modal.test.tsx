...
const makeGuardrailDef = (
  name: string,
  description = "A guardrail description",
) => ({
  guardrail_name: name,
...
const defaultProps = {
  ...
  template: makeTemplate([
    makeGuardrailDef("guardrail-new-1"),
    makeGuardrailDef("guardrail-new-2"),
  ]),
...
describe("GuardrailSelectionModal", () => {
  ...
  it("should not show a checkbox for guardrails that already exist", async () => {
    const props = {
      ...defaultProps,
      template: makeTemplate([
        makeGuardrailDef("existing-g"),
        makeGuardrailDef("new-g"),
      ]),
  ...
  it("should show 'Create N Guardrails & Use Template' on the confirm button when N guardrails are selected", async () => {
    ...
    expect(
      await screen.findByRole("button", {
        name: /create 2 guardrails & use template/i,
      }),
    ).toBeInTheDocument();
  });
  ...
  it("should call onConfirm with the definitions of selected guardrails when confirmed", async () => {
    ...
    renderWithProviders(<GuardrailSelectionModal {...props} />);
    await user.click(
      await screen.findByRole("button", { name: /create 1 guardrail/i }),
    );
  ...
  it("should show 'No guardrails defined' when the template has no guardrail definitions", async () => {
    ...
    renderWithProviders(<GuardrailSelectionModal {...props} />);
    expect(
      await screen.findByText(/no guardrails defined for this template/i),
    ).toBeInTheDocument();
  });
