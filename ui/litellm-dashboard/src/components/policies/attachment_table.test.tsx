...
describe("AttachmentTable", () => {
  ...
  it("should render a row for each attachment", () => {
    const attachments = [
      makeAttachment({
        attachment_id: "att-aaa0001",
        policy_name: "policy-alpha",
      }),
      makeAttachment({
        attachment_id: "att-bbb0002",
        policy_name: "policy-beta",
      }),
    ];
    renderWithProviders(
      <AttachmentTable {...defaultProps} attachments={attachments} />,
    );
  ...
  it("should show team tags when the attachment has teams", () => {
    const attachments = [
      makeAttachment({ teams: ["team-alpha", "team-beta"] }),
    ];
    renderWithProviders(
      <AttachmentTable {...defaultProps} attachments={attachments} />,
    );
  ...
  it("should call onDeleteClick with the attachment_id when the delete icon is clicked", async () => {
    ...
    renderWithProviders(
      <AttachmentTable {...defaultProps} attachments={[attachment]} />,
    );
