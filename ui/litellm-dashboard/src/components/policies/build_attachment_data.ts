import type { PolicyAttachmentCreateRequest } from "./types";
...
export function buildAttachmentData(
  ...
  if (scopeType === "global") {
    ...
    if (formValues.teams && formValues.teams.length > 0)
      data.teams = formValues.teams;
    if (formValues.keys && formValues.keys.length > 0)
      data.keys = formValues.keys;
    if (formValues.models && formValues.models.length > 0)
      data.models = formValues.models;
    if (formValues.tags && formValues.tags.length > 0)
      data.tags = formValues.tags;
  }
  return data;
}
