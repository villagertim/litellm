...
describe("SurveyModal", () => {
  ...
  it("should enable the Next button after selecting Yes", async () => {
    ...
    await user.click(
      screen.getByRole("button", { name: /We use it for work/i }),
    );
  ...
  it("should skip the company name step when No is selected and go straight to step 3", async () => {
    ...
    expect(
      screen.getByText(/When did you start using LiteLLM\?/i),
    ).toBeInTheDocument();
  });
  ...
  describe("when step 4 (reasons) is reached", () => {
    async function navigateToStep4(user: ReturnType<typeof userEvent.setup>) {
      // No path: step 1 → 3 → 4
      await user.click(
        screen.getByRole("button", { name: /Personal project/i }),
      );
      await user.click(screen.getByRole("button", { name: /Next/i }));
      await user.click(
        screen.getByRole("radio", { name: /Less than a month ago/i }),
      );
      await user.click(screen.getByRole("button", { name: /Next/i }));
    }
    ...
    it("should show a text input when the Other reason is selected", async () => {
      ...
      await user.click(
        screen.getByRole("button", {
          name: /Something else not listed above/i,
        }),
      );

      expect(
        screen.getByPlaceholderText(/Please specify/i),
      ).toBeInTheDocument();
    });
    ...
    it("should enable Next when a standard reason is selected", async () => {
      ...
      await user.click(
        screen.getByRole("button", {
          name: /Stars, contributors, forks, community support/i,
        }),
      );
