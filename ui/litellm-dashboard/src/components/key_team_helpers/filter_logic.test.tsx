...
const makeApiResponse = (
  overrides: { keys?: any[]; total_count?: number; total_pages?: number } = {},
) => ({
...
describe("useFilterLogic – filteredTotalCount", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(keyListCall).mockResolvedValue(
      makeApiResponse({ total_count: 509, total_pages: 11 }),
    );
  });
  ...
  it("should set filteredTotalCount to the API total_count after a Key Alias filter is applied", async () => {
    vi.mocked(keyListCall).mockResolvedValue(
      makeApiResponse({ keys: [mockKey], total_count: 1, total_pages: 1 }),
    );
  ...
  it("should reflect the filtered total_count even when it differs from the full key count", async () => {
    vi.mocked(keyListCall).mockResolvedValue(
      makeApiResponse({ total_count: 7, total_pages: 1 }),
    );
