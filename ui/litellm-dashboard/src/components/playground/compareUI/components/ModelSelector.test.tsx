...
describe("ModelSelector", () => {
  it("should render", () => {
    ...
    const { container } = render(
      <ModelSelector value="" onChange={onChange} models={models} />,
    );
  ...
  it("allows selecting a model and displays custom values", async () => {
    ...
    await waitFor(() => {
      const gpt4Option = document.querySelector(
        '[title="gpt-4"].ant-select-item-option',
      ) as HTMLElement;
    ...
    const { container: container2, rerender } = render(
      <ModelSelector
        value="custom-model-123"
        onChange={onChange}
        models={models}
      />,
    );
    const selectedValue = container2.querySelector(
