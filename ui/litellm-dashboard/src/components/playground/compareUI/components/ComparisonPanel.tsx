...
export function ComparisonPanel({
  ...
  const currentSelection = getComparisonSelection(
    comparison,
    endpointConfig.id,
  );
  ...
  const handleAdvancedParamsChange = (checked: boolean) => {
    onUpdate(
      ...
      comparison.applyAcrossModels
        ? { applyToAll: true, keysToApply: ["useAdvancedParams"] }
        : undefined,
    );
  };

  const handleSettingChange = <K extends keyof ComparisonInstance>(
    key: K,
    value: ComparisonInstance[K],
  ) => {
    onUpdate(
      {
        [key]: value,
      ...
      comparison.applyAcrossModels
        ? { applyToAll: true, keysToApply: [key] }
        : undefined,
    );
  };
            ...
            >
              <div>
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className={`text-xs ${disabledTextColor}`}>
