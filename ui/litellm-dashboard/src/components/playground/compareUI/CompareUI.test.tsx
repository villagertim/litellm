...
vi.mock("./components/MessageInput", () => ({
  MessageInput: ({
    ...
    <div data-testid="message-input">
      {uploadComponent && (
        <div data-testid="upload-component">{uploadComponent}</div>
      )}
      <textarea
...
describe("CompareUI", () => {
  ...
  it("adds a comparison when Add Comparison button is clicked", async () => {
    ...
    expect(getByTestId("comparison-panel-1")).toBeInTheDocument();
    expect(getByTestId("comparison-panel-2")).toBeInTheDocument();
    let comparisonPanels = container.querySelectorAll(
      '[data-testid^="comparison-panel-"]',
    );
    expect(comparisonPanels).toHaveLength(2);

    const addButtons = Array.from(
      container.querySelectorAll('button[class*="ant-btn"]'),
    );
    const addComparisonButton = addButtons.find((btn) =>
      btn.textContent?.includes("Add Comparison"),
    );
    ...
    await waitFor(() => {
      comparisonPanels = container.querySelectorAll(
        '[data-testid^="comparison-panel-"]',
      );
