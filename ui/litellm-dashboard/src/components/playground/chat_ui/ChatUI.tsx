...
import MCPToolArgumentsForm, {
  type MCPToolArgumentsFormRef,
} from "../../mcp_tools/MCPToolArgumentsForm";
...
import { type Agent, fetchAvailableAgents } from "../llm_calls/fetch_agents";
import {
  type ModelGroup,
...
import { A2ATaskMetadata, type MessageType } from "./types";
...
const ChatUI: React.FC<ChatUIProps> = ({
  ...
  const [isToolsetsInfoModalVisible, setIsToolsetsInfoModalVisible] =
    useState(false);
  const [byokModalServer, setByokModalServer] = useState<MCPServer | null>(
    null,
  );
  const [selectedMCPServers, setSelectedMCPServers] = useState<string[]>(() => {
    ...
    try {
      ...
      console.error(
        "Error parsing selectedMCPServers from sessionStorage",
        error,
      );
      return [];
    }
  ...
  const [selectedMCPDirectTool, setSelectedMCPDirectTool] = useState<
    string | undefined
  >(undefined);
  ...
  >(() => {
    ...
    try {
      ...
      console.error(
        "Error parsing mcpServerToolRestrictions from sessionStorage",
        error,
      );
      return {};
    }
  ...
  const [selectedModel, setSelectedModel] = useState<string | undefined>(
    simplified ? fixedModel : undefined,
  );
  ...
  const [selectedVectorStores, setSelectedVectorStores] = useState<string[]>(
    () => {
      ...
      try {
        ...
        console.error(
          "Error parsing selectedVectorStores from sessionStorage",
          error,
        );
        return [];
      }
  ...
  const [selectedGuardrails, setSelectedGuardrails] = useState<string[]>(() => {
    ...
    try {
      ...
      console.error(
        "Error parsing selectedGuardrails from sessionStorage",
        error,
      );
      return [];
    }
  ...
  const [selectedPolicies, setSelectedPolicies] = useState<string[]>(() => {
    ...
    try {
      ...
      console.error(
        "Error parsing selectedPolicies from sessionStorage",
        error,
      );
      return [];
    }
  ...
  const [responsesUploadedImage, setResponsesUploadedImage] =
    useState<File | null>(null);
  const [responsesImagePreviewUrl, setResponsesImagePreviewUrl] = useState<
    string | null
  >(null);
  const [chatUploadedImage, setChatUploadedImage] = useState<File | null>(null);
  const [chatImagePreviewUrl, setChatImagePreviewUrl] = useState<string | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    sessionStorage.setItem(
      "mcpServerToolRestrictions",
      JSON.stringify(mcpServerToolRestrictions),
    );
  ...
  useEffect(() => {
    ...
    if (!userApiKey || !token || !userRole || !userID) {
      console.log(
        "userApiKey or token or userRole or userID is missing = ",
        userApiKey,
        token,
        userRole,
        userID,
    ...
    const loadModels = async () => {
      try {
        ...
        const hasSelection = uniqueModels.some(
          (m) => m.model_group === selectedModel,
        );
      ...
      if (selected.startsWith("toolset:")) {
        ...
        if (toolset) {
          const uniqueServerIds = [
            ...new Set(toolset.tools.map((t) => t.server_id)),
          ];
  ...
  useEffect(() => {
    ...
    const loadAgents = async () => {
      try {
        ...
        if (
          selectedAgent &&
          !agents.some((a) => a.agent_name === selectedAgent)
        ) {
  ...
  const handleSendMessage = async () => {
    ...
    if (
      ...
      NotificationsManager.fromBackend(
        "Please upload at least one image for editing",
      );
      return;
    }
    ...
    if (endpointType === EndpointType.TRANSCRIPTION && !uploadedAudio) {
      NotificationsManager.fromBackend(
        "Please upload an audio file for transcription",
      );
      return;
    }
    ...
    if (endpointType === EndpointType.A2A_AGENTS && !selectedAgent) {
      NotificationsManager.fromBackend(
        "Please select an agent to send a message",
      );
      return;
    }
    ...
    if (endpointType === EndpointType.MCP) {
      ...
      const mcpServerId = rawSelected.startsWith("toolset:")
        ? rawSelected
        : rawSelected;
      ...
      const toolsetForSelected = rawSelected.startsWith("toolset:")
        ? mcpToolsets.find(
            (t) => t.toolset_id === rawSelected.slice("toolset:".length),
          )
        : null;
      let searchPool: any[] = [];
      if (toolsetForSelected) {
        const uniqueServerIds = [
          ...new Set(toolsetForSelected.tools.map((t) => t.server_id)),
        ];
        uniqueServerIds.forEach((sid) => {
      ...
      try {
        mcpToolArguments =
          (await mcpToolArgsFormRef.current?.getSubmitValues()) ?? {};
      } catch (err) {
        NotificationsManager.fromBackend(
    ...
    if (
      modelRequiredEndpoints.includes(endpointType as EndpointType) &&
      !selectedModel
    ) {
      NotificationsManager.fromBackend(
        "Please select a model before sending a request",
      );
      return;
    }
    ...
    const effectiveApiKey = simplified
      ? accessToken
    ...
    if (endpointType === EndpointType.RESPONSES && responsesUploadedImage) {
      try {
        newUserMessage = await createMultimodalMessage(
          inputMessage,
    ...
    // Handle image for chat completions API
      ...
      try {
        newUserMessage = await createChatMultimodalMessage(
          inputMessage,
          chatUploadedImage,
        );
      } catch (error) {
    ...
    try {
        ...
        if (endpointType === EndpointType.CHAT) {
          ...
          if (uploadedAudio) {
            await makeOpenAIAudioTranscriptionRequest(
              uploadedAudio,
              (transcription, model) =>
                updateTextUI("assistant", transcription, model),
              selectedModel,
              effectiveApiKey,
              selectedTags,
              signal,
      ...
      if (endpointType === EndpointType.MCP) {
        ...
        if (rawSelected?.startsWith("toolset:")) {
          ...
          const toolset = mcpToolsets.find((t) => t.toolset_id === toolsetId);
          const toolEntry = toolset?.tools.find(
            (t) => t.tool_name === selectedMCPDirectTool,
          );
          resolvedServerId = toolEntry?.server_id ?? rawSelected;
        }
        if (
          resolvedServerId &&
          !resolvedServerId.startsWith("toolset:") &&
          ...
          const resultText =
            result?.content?.length > 0
              ? JSON.stringify(
                  result.content
                    .map((c: any) => (c.type === "text" ? c.text : c))
                    .filter(Boolean),
                  null,
                  2,
                )
  ...
  return (
    <div
      className={`w-full bg-white ${simplified ? "h-full flex flex-col" : "p-4 pb-0"}`}
    >
      <Card
        className={`w-full rounded-xl shadow-md overflow-hidden ${simplified ? "h-full flex flex-col" : ""}`}
      >
        <div
          className={`flex w-full gap-4 ${simplified ? "h-full" : "h-[80vh]"}`}
        >
          {/* Left Sidebar with Controls - hidden in simplified mode */}
          {!simplified && (
            <div className="w-1/4 p-4 bg-gray-50 overflow-y-auto">
              <Title className="text-xl font-semibold mb-6 mt-2">
                Configurations
              </Title>
              <div className="space-y-4">
                            ...
                            new Set(
                              modelInfo
                                .filter((option) => {
                                  ...
                                  const optionEndpoint = getEndpointType(
                                    option.mode,
                                  );
                                  ...
                                  if (
                                    ...
                                    return (
                                      optionEndpoint === endpointType ||
                                      optionEndpoint === EndpointType.CHAT
                                    );
                                  }
                                  ...
                                  if (
                                    ...
                                    return (
                                      optionEndpoint === endpointType ||
                                      optionEndpoint === EndpointType.IMAGE
                                    );
                                  }
                ...
                <div>
                  ...
                  <Select
                    mode={
                      endpointType === EndpointType.MCP ? undefined : "multiple"
                    }
                    style={{ width: "100%" }}
                    placeholder={
                      endpointType === EndpointType.MCP
                        ...
                        if ((value as string[]).includes("__all__")) {
                          ...
                          setMCPServerToolRestrictions((prev) => {
                            ...
                            Object.keys(updated).forEach((serverId) => {
                              if (!(value as string[]).includes(serverId))
                    ...
                    maxTagCount={
                      endpointType === EndpointType.MCP ? 1 : "responsive"
                    }
                  ...
                  {endpointType === EndpointType.MCP &&
                    ...
                    (() => {
                      ...
                      if (isToolset) {
                        ...
                        toolOptions = (serverToolsMap[rawSel] || []).map(
                          (tool: any) => ({
                            value: tool.name,
                            label: tool.name,
                          }),
                      ...
                      return (
                        <div className="mt-3">
                          ...
                          <Select
                            ...
                            onChange={(value) =>
                              setSelectedMCPDirectTool(value)
                            }
                            options={toolOptions}
                            allowClear
                            className="rounded-md"
                          ...
                          return (
                            <div key={serverId} className="border rounded p-2">
                              ...
                              <Select
                                ...
                                value={
                                  mcpServerToolRestrictions[serverId] || []
                                }
                    ...
                    selectedMCPServers.some((serverId) => {
                      ...
                      <div className="mt-3 space-y-2">
                        {selectedMCPServers.map((serverId) => {
                          ...
                          const serverName =
                            server.alias || server.server_name || serverId;
                          return (
                            <div
                              key={serverId}
                ...
                <div>
                  <Text className="font-medium block mb-2 text-gray-700 flex items-center">
                    ...
                    <Tooltip
                      ...
                      title={
                        <span>
                          Select vector store(s) to use for this LLM API call.
                          You can set up your vector store{" "}
                          <a
                            href="?page=vector-stores"
                            style={{ color: "#1890ff" }}
                          >
                            here
                          </a>
                ...
                <div>
                  <Text className="font-medium block mb-2 text-gray-700 flex items-center">
                    ...
                    <Tooltip
                      ...
                      title={
                        <span>
                          Select guardrail(s) to use for this LLM API call. You
                          can set up your guardrails{" "}
                          <a
                            href="?page=guardrails"
                            style={{ color: "#1890ff" }}
                          >
                            here
                          </a>
                ...
                <div>
                  <Text className="font-medium block mb-2 text-gray-700 flex items-center">
                    ...
                    <Tooltip
                      ...
                      title={
                        <span>
                          Select policy/policies to apply to this LLM API call.
                          Policies define which guardrails are applied based on
                          conditions. You can set up your policies{" "}
          ...
          {/* Main Chat Area */}
          <div
            className={`flex flex-col bg-white ${simplified ? "flex-1 w-full" : "w-3/4"}`}
          >
                      ...
                      {uploadedImages.length === 0 ? (
                        ...
                        >
                          ...
                          <p className="ant-upload-hint text-xs text-gray-500">
                            Support for PNG, JPG, JPEG formats. Multiple images
                            supported.
                          </p>
                        </Dragger>
                      ) : (
                      ...
                      {!uploadedAudio ? (
                        ...
                        <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg border border-gray-200">
                          <div className="flex items-center gap-2 flex-1">
                            ...
                            <span className="text-xs text-gray-500">
                              ({(uploadedAudio.size / 1024 / 1024).toFixed(2)}{" "}
                              MB)
                            </span>
                          </div>
                          <button
                          ...
                          {(() => {
                            ...
                            if (rawSel.startsWith("toolset:")) {
                              ...
                              if (toolset) {
                                const uniqueServerIds = [
                                  ...new Set(
                                    toolset.tools.map((t) => t.server_id),
                                  ),
                                ];
                                uniqueServerIds.forEach((sid) => {
                                  toolPool = toolPool.concat(
                                    serverToolsMap[sid] || [],
                                  );
                                });
                              }
      ...
      <Modal
        ...
        footer={[
          <Button
            key="close"
            onClick={() => setIsToolsetsInfoModalVisible(false)}
          >
            Close
          </Button>,
        ]}
      ...
      >
        <div className="space-y-4 py-2">
          <p className="text-gray-700">
            <strong>Toolsets</strong> are named collections of specific tools
            from one or more MCP servers. Instead of exposing all tools from a
            server, a toolset gives an agent exactly the tools it needs.
          </p>
          <div>
            ...
            <ol className="list-decimal list-inside space-y-2 text-gray-700">
              <li>
                ...
                (purple badge) from the MCP Servers dropdown.
              </li>
              <li>
                The tool picker will show only the tools included in that
                toolset.
              </li>
              <li>Select a tool and fill in its parameters, then send.</li>
              <li>
                The tool call is routed to the correct underlying MCP server
                automatically.
              </li>
            </ol>
          </div>
          <div className="bg-purple-50 border border-purple-200 rounded p-3">
            <p className="text-sm text-purple-800">
              <strong>Example:</strong> A &quot;GitHub Read-only&quot; toolset
              might include only <code>list_repos</code> and{" "}
              <code>get_file</code> from a GitHub MCP server — preventing agents
              from making writes.
            </p>
          </div>
          <div>
            ...
            <p className="text-sm text-gray-600">
              Admins can create and manage toolsets from the{" "}
              <strong>MCP</strong> page → <strong>Toolsets</strong> tab.
              Toolsets can then be assigned to keys and teams to scope their
              tool access.
            </p>
          </div>
        </div>
