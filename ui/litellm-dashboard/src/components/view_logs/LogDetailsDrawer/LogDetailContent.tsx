...
export function LogDetailContent({
  ...
  const missingData =
    !hasMessages && !hasResponse && !hasError && !isLoadingDetails;
  ...
  return (
    <div
      style={{
        padding: `${DRAWER_CONTENT_PADDING} ${DRAWER_CONTENT_PADDING} 0`,
      }}
    >
      {/* Error Alert */}
      {hasError && errorInfo && (
      ...
      {/* Tags */}
      {logEntry.request_tags &&
        Object.keys(logEntry.request_tags).length > 0 && (
        ...
        >
          <Descriptions column={2} size="small">
            ...
            <Descriptions.Item label="API Base">
              <TruncatedValue
                value={logEntry.api_base}
                maxWidth={API_BASE_MAX_WIDTH}
              />
            </Descriptions.Item>
            ...
            {hasGuardrailData && (
              <Descriptions.Item label="Guardrail">
                <GuardrailLabel
                  label={primaryGuardrailLabel}
                  maskedCount={totalMaskedEntities}
                />
              </Descriptions.Item>
            )}
          </Descriptions>
...
function GuardrailLabel({
  label,
  maskedCount,
}: { label: string; maskedCount: number }) {
  const handleClick = () => {
 ...
 */
function getUncachedInputTextTokens(
...
function MetricsSection({
  logEntry,
  metadata,
}: { logEntry: LogEntry; metadata: Record<string, any> }) {
      ...
      <Card title="Metrics" size="small" style={{ marginBottom: 0 }}>
        <Descriptions column={2} size="small">
          ...
          <Descriptions.Item label="Duration">
            {logEntry.request_duration_ms != null
              ? (logEntry.request_duration_ms / 1000).toFixed(3)
              : "-"}{" "}
            s
          </Descriptions.Item>
          ...
          {metadata?.litellm_overhead_time_ms !== undefined &&
            metadata.litellm_overhead_time_ms !== null && (
...
function RequestResponseSection({
  ...
  const [activeTab, setActiveTab] = useState<
    typeof TAB_REQUEST | typeof TAB_RESPONSE
  >(TAB_REQUEST);
  const [viewMode, setViewMode] = useState<"pretty" | "json">("pretty");

  const getCopyText = () => {
    const data =
      activeTab === TAB_REQUEST ? getRawRequest() : getFormattedResponse();
              ...
              <div>
                {viewMode === "pretty" ? (
                  ...
                  <Tabs
                    ...
                    onChange={(key) =>
                      setActiveTab(
                        key as typeof TAB_REQUEST | typeof TAB_RESPONSE,
                      )
                    }
                    tabBarExtraContent={
                      <Text
...
export function GuardrailJumpLink({
  guardrailEntries,
}: { guardrailEntries: any[] }) {
  ...
  return (
    <div style={{ textAlign: "left", marginBottom: 12 }}>
      ...
      >
        {allPassed ? "\u2713" : "\u2717"} {guardrailEntries.length} guardrail
