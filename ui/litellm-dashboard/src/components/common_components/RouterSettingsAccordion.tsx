...
import type { Fallbacks } from "../Settings/RouterSettings/Fallbacks/AddFallbacks";
...
import RouterSettingsForm, {
  type RouterSettingsFormValue,
} from "../router_settings/RouterSettingsForm";
...
>(({ accessToken, value, onChange, modelData }, ref) => {
  ...
  const [routingStrategyDescriptions, setRoutingStrategyDescriptions] =
  ...
  useEffect(() => {
    ...
    if (
      ...
      valueKey === lastInitializedValueRef.current
    ) {
    ...
    if (value?.router_settings) {
      ...
      setFormValue({
        routerSettings: routerSettingsWithoutFallbacks as {
          [key: string]: any;
        },
  ...
  const buildRouterSettings =
    (): RouterSettingsAccordionValue["router_settings"] => {
        ...
        Object.entries(settingsToUpdate)
          .map(([key, value]) => {
            if (
              ...
              key === "routing_strategy_args" &&
              formValue.selectedStrategy === "latency-based-routing"
            ) {
              ...
              if (lowestLatencyBufferElement?.value) {
                routingStrategyArgs["lowest_latency_buffer"] = Number(
              ...
              return [
                ...
                Object.keys(routingStrategyArgs).length > 0
                  ? routingStrategyArgs
                  : null,
              ];
            }
      ...
      const normalizeValue = (val: any, isNumber = false): any => {
        ...
        if (
          ...
          return null;
        if (isNumber && (typeof val !== "number" || Number.isNaN(val)))
          return null;
        return val;
      };

      return {
        routing_strategy: normalizeValue(updatedVariables.routing_strategy) as
          | string
          | null
          | undefined,
        allowed_fails: normalizeValue(updatedVariables.allowed_fails, true) as
          | number
          | null
          | undefined,
        cooldown_time: normalizeValue(updatedVariables.cooldown_time, true) as
          | number
          | null
          | undefined,
        num_retries: normalizeValue(updatedVariables.num_retries, true) as
          | number
          | null
          | undefined,
        timeout: normalizeValue(updatedVariables.timeout, true) as
          | number
          | null
          | undefined,
        retry_after: normalizeValue(updatedVariables.retry_after, true) as
          | number
          | null
          | undefined,
        fallbacks: fallbacks.length > 0 ? fallbacks : null,
        context_window_fallbacks: normalizeValue(
          updatedVariables.context_window_fallbacks,
        ),
        ...
        enable_tag_filtering: formValue.enableTagFiltering,
        routing_strategy_args: normalizeValue(
          updatedVariables.routing_strategy_args,
        ),
      };
    };
