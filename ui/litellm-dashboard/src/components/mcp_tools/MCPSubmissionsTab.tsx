...
import type { MCPServer, MCPSubmissionsSummary } from "./types";
      ...
      <div className="bg-white rounded-xl shadow-xl p-6 max-w-sm w-full mx-4">
        ...
        <div className="flex gap-3">
          ...
          <button
            type="button"
            onClick={() =>
              onConfirm(isApprove ? undefined : reviewNotes || undefined)
            }
...
function SubmissionRulesPanel({
  requiredFields,
  onChange,
  onSave,
  isSaving,
}: SubmissionRulesPanelProps) {
  const [expanded, setExpanded] = useState(false);
  const activeLabels = MCP_REQUIRED_FIELD_DEFS.filter((f) =>
        ...
        <div className="flex items-center gap-2">
          ...
          {activeLabels.length > 0 ? (
            <span className="text-xs text-gray-500">
              ({activeLabels.length} required field
              {activeLabels.length !== 1 ? "s" : ""})
            </span>
          ) : (
      ...
      {expanded && (
        <div className="border-t border-gray-100 px-4 pt-4 pb-4">
          <p className="text-xs text-gray-500 mb-4">
            Select which fields must be filled in before a submission is
            considered compliant. LiteLLM will show ✓ / ✗ for each rule on every
            submission card below.
          </p>
...
function MCPServerCard({
  ...
  const checks = MCP_REQUIRED_FIELD_DEFS.filter((f) =>
    requiredFields.includes(f.key),
  ).map((f) => ({
    key: f.key,
    label: f.label,
      ...
      {checks.length > 0 && (
        <div className="border-t border-gray-200">
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <div
                className={`text-sm font-semibold leading-tight ${allPassed ? "text-green-800" : "text-red-800"}`}
              >
                {allPassed
                  ? "All checks passed"
          ...
          <div className="divide-y divide-gray-100">
            {checks.map((c) => (
              <div key={c.key} className="flex items-center gap-3 px-4 py-2.5">
                ...
                <span
                  className={`text-sm flex-1 ${c.passed ? "text-gray-700" : "text-gray-800"}`}
                >
                  {c.label}
                </span>
                <span
                  className={`text-xs ${c.passed ? "text-green-600" : "text-red-500"}`}
                >
...
export function MCPSubmissionsTab({ accessToken }: MCPSubmissionsTabProps) {
    ...
    try {
      const [res, settings] = await Promise.all([
        ...
        getGeneralSettingsCall(accessToken).catch((err) => {
          console.warn(
            "MCPSubmissionsTab: failed to load general settings, compliance rules will be empty:",
            err,
          );
          return null;
        }),
      ...
      if (settings?.data && Array.isArray(settings.data)) {
        const row = settings.data.find(
          (r: { field_name: string; field_value: unknown }) =>
            r.field_name === SETTINGS_KEY,
        );
  ...
  const filtered = summary.items.filter((s) => {
    if (statusFilter !== "all" && s.approval_status !== statusFilter)
      return false;
    if (search.trim()) {
