          ...
          const userData = results.data
            ...
            .map((row: any, index: number) => {
              ...
              if (user.max_budget && user.max_budget.toString().trim() !== "") {
                if (isNaN(Number.parseFloat(user.max_budget.toString()))) {
                  errors.push(
                    `Max budget "${user.max_budget}" must be a number`,
                  );
                } else if (Number.parseFloat(user.max_budget.toString()) <= 0) {
  ...
  const handleBulkCreate = async () => {
    ...
    for (let index = 0; index < updatedData.length; index++) {
      ...
      try {
        ...
        if (user.max_budget && user.max_budget.toString().trim() !== "") {
          const budgetValue = Number.parseFloat(user.max_budget.toString());
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    <div className="flex items-start">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-600">
                          User&apos;s role (one of: &quot;proxy_admin&quot;,
                          &quot;proxy_admin_viewer&quot;,
                          &quot;internal_user&quot;,
                          &quot;internal_user_viewer&quot;)
                        </p>
                      ...
                      <div className="flex items-center">
                        ...
                        <div>
                          <Typography.Text
                            strong
                            className={
                              fileError ? "text-red-800" : "text-blue-800"
                            }
                          >
                            {selectedFile.name}
                          </Typography.Text>
                          <Typography.Text
                            className={`block text-xs ${fileError ? "text-red-600" : "text-blue-600"}`}
                          >
                    ...
                    <div>
                      ...
                      {parsedData.some((user) => !user.isValid) && (
                        <ul className="mt-2 list-disc list-inside text-red-600 text-sm">
                          ...
                          <li>
                            Common issues include invalid email formats, missing
                            required fields, or incorrect role values
                          </li>
                          <li>
                      ...
                      <div className="flex items-center">
                        ...
                        {parsedData.some((d) => d.status === "failed") && (
                          <Text className="text-sm bg-red-100 text-red-800 px-2 py-1 rounded">
                            {
                              parsedData.filter((d) => d.status === "failed")
                                .length
                            }{" "}
                            Failed
                          </Text>
                    ...
                    <div className="flex items-start">
                      ...
                      <div>
                        ...
                        <Text className="block text-sm text-blue-700 mt-1">
                          ...
                          Download the credentials file containing Virtual Keys
                          and invitation links. Users will need these Virtual
                          Keys to make LLM requests through LiteLLM.
                        </Text>
                      </div>
                    </div>
                  </div>
