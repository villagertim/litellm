...
const ChatPage: React.FC<ChatPageProps> = ({
  ...
  const [comparisonExchanges, setComparisonExchanges] = useState<
    ComparisonExchange[]
  >([]);
  const [comparisonStreamingSet, setComparisonStreamingSet] = useState<
    Set<string>
  >(new Set());
        ...
        try {
          ...
          if (saved) {
            ...
            if (Array.isArray(parsed)) {
              const valid = (parsed as string[]).filter((m) =>
                names.includes(m),
              );
              if (valid.length > 0) {
    ...
    async (
      ...
      try {
        ...
        if (err instanceof Error && err.name === "AbortError") {
          ...
          updateLastAssistantMessage(convId!, {
            content:
              "[Something went wrong. The partial response has been saved.]",
          });
        }
      } finally {
        ...
        if (accumulatedMCPEvents.length > 0 && streamCompletedCleanly) {
          updateLastAssistantMessage(convId!, {
            mcpEvents: accumulatedMCPEvents,
          });
        }
  ...
  const handleSendComparison = useCallback(
    (text: string, currentExchanges: ComparisonExchange[]) => {
      ...
      const newExchange: ComparisonExchange = {
        userMessage: trimmed,
        responses: {},
      };
  ...
  const handleStop = useCallback(() => {
    ...
    Object.values(comparisonAbortControllersRef.current).forEach((c) =>
      c.abort(),
    );
    ...
    >
      ...
      <div style={{ flex: 1, overflowY: "auto" }}>
        {filteredModels.map((m) => {
          ...
          const disabled =
            !checked && selectedModels.length >= MAX_COMPARISON_MODELS;
          const provider = getProviderFromModelName(m);
          const { logo } = provider
            ? getProviderLogoAndName(provider)
            : { logo: "" };
          return (
            <button
              key={m}
      ...
      >
        ...
        {!sidebarCollapsed && (
          <>
            ...
            {kbd && (
              <span style={{ fontSize: 11, color: "#9ca3af" }}>{kbd}</span>
            )}
          </>
        )}
      </button>
  ...
  const inputBar = (inConversation: boolean) => (
    ...
    >
      <textarea
        ...
        placeholder={
          inConversation ? "Send a message..." : "How can I help you today?"
        }
        style={{
          width: "100%",
    ...
    >
      ...
      >
        ...
        <div style={{ padding: "0 8px 4px", flexShrink: 0 }}>
          {sidebarNavItem(<EditOutlined />, "New chat", () =>
            router.push(getChatUrl(uiRoot)),
          )}
          {sidebarNavItem(<SearchOutlined />, "Search chats", () =>
        ...
        <div style={{ padding: "4px 8px", flexShrink: 0 }}>
          ...
          <Tooltip
            title={
              sidebarCollapsed ? "Back to Developer Console UI" : undefined
            }
            placement="right"
          >
            <a
      ...
      >
        ...
        {storageUnavailable && !storageBannerDismissed && (
          ...
          >
            <span>
              Chat history won&apos;t be saved in this browser session.
            </span>
            <button
          ...
          {sidebarView === "apps" ? (
            ...
            >
              ...
              {isComparisonMode ? (
                ...
                >
                  Chat with 100+ LLMs + MCP tools — authenticate once, use them
                  here.{" "}
                  <button
                  ...
                  <div style={{ paddingBottom: 8 }}>
                    {comparisonExchanges.map((exchange, exchangeIdx) => {
                      const isLastExchange =
                        exchangeIdx === comparisonExchanges.length - 1;
                      return (
                          ...
                          >
                            {selectedModels.map((model, idx) => {
                              ...
                              const { logo } = provider
                                ...
                                : { logo: "" };
                              const responseText =
                                exchange.responses[model] ?? "";
                              const isModelStreaming =
                                isLastExchange &&
                                comparisonStreamingSet.has(model);
                              return (
                                <div
                                  key={model}
                                  style={{
