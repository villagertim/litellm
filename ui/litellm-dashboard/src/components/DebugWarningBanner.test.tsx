...
describe("DebugWarningBanner", () => {
  it("should render", () => {
    vi.mocked(useHealthReadinessDetails).mockReturnValue({
      data: { is_detailed_debug: true },
    } as any);
  ...
  it("should show warning when detailed debug mode is active", () => {
    ...
    renderWithProviders(<DebugWarningBanner accessToken="token" />);
    expect(
      screen.getByText(/Performance Warning: Detailed Debug Mode Active/i),
    ).toBeInTheDocument();
  });
  ...
  it("should render nothing when is_detailed_debug is false", () => {
    vi.mocked(useHealthReadinessDetails).mockReturnValue({
      data: { is_detailed_debug: false },
    } as any);
    const { container } = renderWithProviders(
      <DebugWarningBanner accessToken="token" />,
    );
