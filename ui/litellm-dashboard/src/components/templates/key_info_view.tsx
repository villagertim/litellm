...
export default function KeyInfoView({
  ...
  const { mutate: resetKeySpend, isPending: resetSpendLoading } =
    useResetKeySpend();
  // Add local state to maintain key data and track regeneration
  const [currentKeyData, setCurrentKeyData] = useState<KeyResponse | undefined>(
    keyData,
  );
  ...
  const handleKeyUpdate = async (formValues: Record<string, any>) => {
    try {
      ...
      for (const field of PREMIUM_METADATA_FIELDS) {
        const previousValue =
          (currentKeyData.metadata as Record<string, unknown> | undefined)?.[
            field
      ...
      if (formValues.mcp_servers_and_groups !== undefined) {
        const { servers, accessGroups, toolsets } =
          formValues.mcp_servers_and_groups || {
            servers: [],
            accessGroups: [],
            toolsets: [],
          };
      ...
      if (formValues.agents_and_groups !== undefined) {
        const { agents, accessGroups } = formValues.agents_and_groups || {
          agents: [],
          accessGroups: [],
        };
        formValues.object_permission = {
      ...
      formValues.max_parallel_requests = mapEmptyStringToNull(
        formValues.max_parallel_requests,
      );
      ...
      if (formValues.metadata && typeof formValues.metadata === "string") {
        try {
          ...
          formValues.metadata = {
            ...
            formValues.logging_settings.length > 0
              ? { logging: formValues.logging_settings }
              : {}),
            ...(formValues.disabled_callbacks?.length > 0
              ? {
                  litellm_disabled_callbacks: mapDisplayToInternalNames(
                    formValues.disabled_callbacks,
                  ),
                }
              : {}),
          };
      ...
      setCurrentKeyData((prevData) =>
        prevData ? { ...prevData, ...newKeyValues } : undefined,
      );
  ...
  const handleDelete = async () => {
    try {
      ...
      await keyDeleteCall(
        ...
        currentKeyData.token || currentKeyData.token_id,
      );
  ...
  const handleResetSpend = () => {
    resetKeySpend(currentKeyData.token || currentKeyData.token_id, {
      onSuccess: () => {
        setCurrentKeyData((prevData) =>
          prevData ? { ...prevData, spend: 0 } : undefined,
        );
        if (onKeyDataUpdate) {
    ...
    <div className="w-full h-full overflow-y-auto p-4">
      <KeyInfoHeader
        data={{
          ...
          createdBy:
            ...
            "",
          createdAt: currentKeyData.created_at
            ? formatTimestamp(currentKeyData.created_at)
            : "",
          lastUpdated: currentKeyData.updated_at
            ? formatTimestamp(currentKeyData.updated_at)
            : "",
        ...
        onResetSpend={
          canResetSpend ? () => setIsResetSpendModalOpen(true) : undefined
        }
      ...
      >
        ...
        <p style={{ color: "#666", fontSize: "0.875rem", marginTop: 8 }}>
          Current spend:{" "}
          <strong>${formatNumberWithCommas(currentKeyData.spend, 4)}</strong>.
          Spend history is preserved in logs. This resets the current period
          spend counter, the same as an automatic budget reset.
        </p>
      </Modal>
          ...
          <TabPanel>
            <Grid numItems={1} numItemsSm={2} numItemsLg={3} className="gap-6">
              ...
              <Card>
                ...
                {Array.isArray(currentKeyData.metadata?.guardrails) &&
                currentKeyData.metadata.guardrails.length > 0 ? (
                  <div className="flex flex-wrap gap-2">
                    {currentKeyData.metadata.guardrails.map(
                ...
                {typeof currentKeyData.metadata?.disable_global_guardrails ===
                  "boolean" &&
              ...
              <Card>
                ...
                {Array.isArray(currentKeyData.metadata?.policies) &&
                currentKeyData.metadata.policies.length > 0 ? (
                  <div className="space-y-4">
                    {currentKeyData.metadata.policies.map(
              ...
              <LoggingSettingsView
                ...
                disabledCallbacks={
                  Array.isArray(
                    currentKeyData.metadata?.litellm_disabled_callbacks,
                  )
                    ? mapInternalToDisplayNames(
                        currentKeyData.metadata.litellm_disabled_callbacks,
                      )
              ...
              {isEditing ? (
                ...
                <div className="space-y-4">
                        ...
                        {currentKeyData.project_id
                          ? (() => {
                              const project = projects?.find(
                                (p) =>
                                  p.project_id === currentKeyData.project_id,
                              );
                  ...
                  <div>
                    ...
                    <Text>
                      {(currentKeyData.organization_id ??
                        currentKeyData.org_id) ||
                        "Not Set"}
                    </Text>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2 mt-1">
                      {Array.isArray(currentKeyData.metadata?.tags) &&
                      currentKeyData.metadata.tags.length > 0
                        ? currentKeyData.metadata.tags.map((tag, index) => (
                            <span
                              key={index}
                              className="px-2 mr-2 py-1 bg-blue-100 rounded text-xs"
                            >
                              {tag}
                            </span>
                          ))
                  ...
                  <div>
                    ...
                    <Text>
                      {Array.isArray(currentKeyData.metadata?.prompts) &&
                      currentKeyData.metadata.prompts.length > 0
                        ? currentKeyData.metadata.prompts.map(
                            (prompt, index) => (
                              <span
                                key={index}
                                className="px-2 mr-2 py-1 bg-blue-100 rounded text-xs"
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2 mt-1">
                      {Array.isArray(currentKeyData.allowed_routes) &&
                      currentKeyData.allowed_routes.length > 0 ? (
                  ...
                  <div>
                    ...
                    <Text>
                      {Array.isArray(
                        currentKeyData.metadata?.allowed_passthrough_routes,
                      ) &&
                      currentKeyData.metadata.allowed_passthrough_routes
                        .length > 0
                        ? currentKeyData.metadata.allowed_passthrough_routes.map(
                            (route, index) => (
                              <span
                                key={index}
                                className="px-2 mr-2 py-1 bg-blue-100 rounded text-xs"
                  ...
                  <div>
                    ...
                    <pre className="bg-gray-100 p-2 rounded text-xs overflow-auto mt-1">
                      {formatMetadataForDisplay(
                        stripTagsFromMetadata(currentKeyData.metadata),
                      )}
                    </pre>
                  </div>
