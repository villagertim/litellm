...
export default function UISettings() {
  ...
  const {
    ...
    error: updateError,
  } = useUpdateUISettings(accessToken);

  const schema = data?.field_schema;
  const property = schema?.properties?.disable_model_add_for_internal_users;
  const disableTeamAdminDeleteProperty =
    schema?.properties?.disable_team_admin_delete_team_user;
  const requireAuthForPublicAIHubProperty =
    schema?.properties?.require_auth_for_public_ai_hub;
  const forwardClientHeadersProperty =
    schema?.properties?.forward_client_headers_to_llm_api;
  ...
  const enabledPagesProperty =
    schema?.properties?.enabled_ui_pages_internal_users;
  const disableAgentsProperty =
    schema?.properties?.disable_agents_for_internal_users;
  const allowAgentsTeamAdminsProperty =
    schema?.properties?.allow_agents_for_team_admins;
  const disableVectorStoresProperty =
    schema?.properties?.disable_vector_stores_for_internal_users;
  const allowVectorStoresTeamAdminsProperty =
    schema?.properties?.allow_vector_stores_for_team_admins;
  const scopeUserSearchProperty = schema?.properties?.scope_user_search_to_org;
  const disableCustomApiKeysProperty =
    schema?.properties?.disable_custom_api_keys;
  const values = data?.values ?? {};
  const isDisabledForInternalUsers = Boolean(
    values.disable_model_add_for_internal_users,
  );
  const isDisabledTeamAdminDeleteTeamUser = Boolean(
    values.disable_team_admin_delete_team_user,
  );
  const isAgentsDisabled = Boolean(values.disable_agents_for_internal_users);
  const isVectorStoresDisabled = Boolean(
    values.disable_vector_stores_for_internal_users,
  );
  ...
  const handleUpdatePageVisibility = (settings: {
    enabled_ui_pages_internal_users: string[] | null;
  }) => {
    updateSettings(settings, {
      onSuccess: () => {
        NotificationManager.success(
          "Page visibility settings updated successfully",
        );
      },
      onError: (error) => {
        NotificationManager.fromBackend(error);
  ...
  const handleToggleEnableProjectsUI = (checked: boolean) => {
    updateSettings(
      { enable_projects_ui: checked },
      {
        onSuccess: () => {
          NotificationManager.success(
            "UI settings updated successfully. Refreshing page...",
          );
        ...
        <Space direction="vertical" size="large" style={{ width: "100%" }}>
          ...
          <Space align="start" size="middle">
            <Switch
              ...
              aria-label={
                property?.description ?? "Disable model add for internal users"
              }
            />
          ...
          {enableProjectsUIProperty && (
            <Space align="start" size="middle">
              <Switch
                ...
                aria-label={
                  enableProjectsUIProperty.description ?? "Enable Projects UI"
                }
              />
              <Space direction="vertical" size={4}>
                <Typography.Text strong>
          ...
          <Space align="start" size="middle" style={{ marginLeft: 32 }}>
            ...
            <Space direction="vertical" size={4}>
              <Typography.Text
                strong
                type={!isAgentsDisabled ? "secondary" : undefined}
              >
                Allow agents for team admins
              </Typography.Text>
              {allowAgentsTeamAdminsProperty?.description && (
          ...
          <Space align="start" size="middle" style={{ marginLeft: 32 }}>
            ...
            <Space direction="vertical" size={4}>
              <Typography.Text
                strong
                type={!isVectorStoresDisabled ? "secondary" : undefined}
              >
                Allow vector stores for team admins
              </Typography.Text>
              {allowVectorStoresTeamAdminsProperty?.description && (
