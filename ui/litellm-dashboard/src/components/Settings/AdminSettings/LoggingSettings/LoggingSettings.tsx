...
import {
  type StoreRequestInSpendLogsParams,
...
const LoggingSettings: React.FC = () => {
  ...
  const { mutate: deleteField, isPending: isDeletingField } =
    useDeleteProxyConfigField();
  const { data: proxyConfigData, isLoading: isLoadingConfig } = useProxyConfig(
  ...
  const initialValues = useMemo(() => {
    ...
    return {
      ...
      maximum_spend_logs_retention_period:
        retentionPeriodField?.field_value ?? undefined,
    };
  }, [proxyConfigData]);
    ...
    deleteField(
      {
        ...
        field_name:
          GeneralSettingsFieldName.MAXIMUM_SPEND_LOGS_RETENTION_PERIOD,
      },
      {
        onError: (deleteError) =>
          console.warn(
    ...
    <Card title="Logging Settings">
      <Space direction="vertical" size="large" style={{ width: "100%" }}>
        <Typography.Paragraph style={{ marginBottom: 0 }} type="secondary">
          Proxy-wide settings that control how request and response data are
          written to spend logs.
        ...
        >
          ...
          >
            {isLoadingConfig ? (
              ...
              <Switch
                ...
                onChange={(checked) =>
                  form.setFieldValue("store_prompts_in_spend_logs", checked)
                }
              />
            )}
          </Form.Item>
          ...
          <Form.Item
            ...
            tooltip={
              proxyConfigData?.find(
                (f) => f.field_name === "maximum_spend_logs_retention_period",
              )?.field_description ||
