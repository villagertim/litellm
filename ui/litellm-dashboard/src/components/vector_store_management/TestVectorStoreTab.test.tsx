...
vi.mock("./VectorStoreTester", () => ({
  VectorStoreTester: ({
    vectorStoreId,
    accessToken,
  }: { vectorStoreId: string; accessToken: string }) => (
...
describe("TestVectorStoreTab", () => {
  it("should render the component successfully", () => {
    ...
    expect(
      screen.getByText("Choose a vector store to test search queries against"),
    ).toBeInTheDocument();
  });

  it("should show message when no access token", () => {
    render(
      <TestVectorStoreTab accessToken={null} vectorStores={mockVectorStores} />,
    );

    expect(
      screen.getByText("Access token is required to test vector stores."),
    ).toBeInTheDocument();
  });
  ...
  it("should render VectorStoreTester with first vector store by default", () => {
    ...
    expect(screen.getByTestId("tester-vector-store-id")).toHaveTextContent(
      "vs_123",
    );
    expect(screen.getByTestId("tester-access-token")).toHaveTextContent(
      "test-token",
    );
  });
