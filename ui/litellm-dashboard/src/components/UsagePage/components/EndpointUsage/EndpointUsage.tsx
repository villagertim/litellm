...
const EndpointUsage: React.FC<EndpointUsageProps> = ({ userSpendData }) => {
  ...
  const endpointData = useMemo(() => {
    ...
    userSpendData.results.forEach((day) => {
      Object.entries(day.breakdown.endpoints || {}).forEach(
        ([endpoint, metrics]) => {
          ...
          aggregatedEndpoints[endpoint].metrics.prompt_tokens +=
            metrics.metrics.prompt_tokens;
          aggregatedEndpoints[endpoint].metrics.completion_tokens +=
            metrics.metrics.completion_tokens;
          aggregatedEndpoints[endpoint].metrics.total_tokens +=
            metrics.metrics.total_tokens;
          aggregatedEndpoints[endpoint].metrics.api_requests +=
            metrics.metrics.api_requests;
          aggregatedEndpoints[endpoint].metrics.successful_requests +=
            metrics.metrics.successful_requests || 0;
          aggregatedEndpoints[endpoint].metrics.failed_requests +=
            metrics.metrics.failed_requests || 0;
          aggregatedEndpoints[endpoint].metrics.cache_read_input_tokens +=
